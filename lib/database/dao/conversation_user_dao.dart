...
class ConversationUserDao extends DatabaseAccessor<AppDatabase>
  ...
  ConversationUserDao(AppDatabase db) : super(db);

  /// 查询某个会话的所有成员，并且把 user 信息 join 进去
  Future<List<ConversationUser>> getUsersForConversation(int conversationId) async {
  ...
  Future<void> replaceConversationUsers(
      int conversationId, List<ConversationUser> users) async {
    await transaction(() async {
      // 删除旧的成员记录
      await (delete(conversationUsers)
        ..where(
                (tbl) => tbl.conversationId.equals(conversationId)))
          .go();

      // 插入新的成员记录（不包含 user 字段，只存 id / joinedAt）
      await batch((batch) {
        batch.insertAll(
          conversationUsers,
          users
  ...
  Future<void> upsertUsersInfoFromConversationUsers(
      List<ConversationUser> users) async {
